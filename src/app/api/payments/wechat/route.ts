 ...
 *
 * WeChat Pay only settles in CNY, so unlike a same-currency gateway this
 * route still must convert — but the order lookup + amount comparison
 * below happens BEFORE any conversion, against the DB-sourced grand_total,
 * so a client-supplied amount can never influence what actually gets charged.
 */
export async function POST(request: NextRequest) {
  ...
  // WeChat Pay always settles in CNY — convert the DB-sourced amount, never the client-supplied value
