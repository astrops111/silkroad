...
export default function PromoteClient({
  ...
  const loadPromotions = useCallback(() => {
    setLoading(true);
    return fetch(`/api/supplier/promote?companyId=${companyId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const alreadyPromoted =
    !!initialProductId && activePromos.some((p) => p.products?.id === initialProductId);
  ...
  async function handlePromoteNow() {
    ...
    try {
      const res = await fetch("/api/supplier/promote", {
        ...
        body: JSON.stringify({ productId: initialProductId, companyId, placement }),
      });
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.error ?? "Failed to promote product");
        return;
      }
      toast.success("Product promoted!");
      await loadPromotions();
      router.replace("/supplier/products/promote");
    } catch {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Promote-this-product panel, shown when arriving from a product row action */}
      {initialProductId && !alreadyPromoted && (
        <div className="rounded-2xl border p-5 space-y-4" style={{ background: "var(--surface-primary)", borderColor: "var(--amber)" }}>
          <div className="flex items-center gap-2">
            <Zap className="w-4 h-4" style={{ color: "var(--amber)" }} />
            <h2 className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Promote this product</h2>
          </div>
          {limit > 0 && used >= limit ? (
            <p className="flex items-center gap-2 text-sm" style={{ color: "var(--warning)" }}>
              <AlertTriangle className="w-4 h-4 shrink-0" /> Monthly promotion quota reached ({used}/{limit}).
            </p>
          ) : (
            <>
              ...
              <button
                ...
                disabled={promoting}
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-full bg-[var(--amber)] text-white hover:opacity-90 transition-opacity disabled:opacity-50"
              >
                {promoting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Megaphone className="w-4 h-4" />}
                {promoting ? "Promoting…" : "Activate Promotion"}
              </button>
            </>
          )}
        </div>
      ...
      {initialProductId && alreadyPromoted && (
        <div className="rounded-2xl border p-4 flex items-center gap-2 text-sm" style={{ background: "var(--surface-primary)", borderColor: "var(--border-subtle)", color: "var(--success)" }}>
          <CheckCircle2 className="w-4 h-4 shrink-0" /> This product already has an active promotion.
        </div>
      )}
