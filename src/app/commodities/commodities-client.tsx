...
/* ============================================================
   HERO — Commodities portal (Africa → World)
   ============================================================ */
...
/* ============================================================
   LIVE COMMODITY LISTINGS (real data — empty state when none)
   ============================================================ */
function LiveListingsSection({ products }: { products: RailProduct[] }) {
  ...
  if (products.length === 0) {
    return (
      <section className="py-16 lg:py-20 bg-[var(--surface-primary)]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="rounded-2xl border border-dashed border-[var(--border-default)] py-16 px-8 text-center">
            <Package className="w-8 h-8 mx-auto text-[var(--text-tertiary)]" />
            <h3
              className="mt-4 text-lg font-bold text-[var(--obsidian)]"
              style={{ fontFamily: "var(--font-display)" }}
            >
              No live listings yet
            </h3>
            <p className="mt-2 text-sm text-[var(--text-secondary)] max-w-md mx-auto">
              Be the first supplier to list a resource on the platform — it&apos;ll appear here
              once approved.
            </p>
            <Link
              href="/auth/register?role=supplier"
              className="btn-primary !text-sm !py-3 !px-5 mt-5 inline-flex"
            >
              List a resource
              <ArrowUpRight className="w-4 h-4" />
            </Link>
...
export default function CommoditiesClient({ products }: { products: RailProduct[] }) {
  ...
  return (
    <>
      ...
      <main className="flex-1">
        <HeroSection />
        <FeaturedCategories />
        <LiveListingsSection products={products} />
        <EditorialBand
