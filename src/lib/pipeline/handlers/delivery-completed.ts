...
export const handler: EventHandler = async (event, supabase) => {
  ...
  await supabase.from("b2b_shipments").update({
    status:             "delivered",
    delivered_at:       now,
    pod_photo_url:      p.podPhotoUrl      ?? null,
  ...
  const allDone = siblings?.every((s) => s.status === "delivered" || s.status === "completed");
  if (allDone) {
    await supabase
  ...
  await onShipmentDelivered(shipment_id)
    .catch((err) => console.error("[pipeline:delivery.completed] Buyer email failed:", err));
