...
export function SuperAdminShell({ children, userName, userInitials }: SuperAdminShellProps) {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar — visible by default; collapses only when the user toggles it */}
      <aside
        className="fixed inset-y-0 left-0 z-40 flex flex-col shrink-0 transition-[width] duration-200"
        style={{ background: "linear-gradient(180deg, #14110F 0%, #1F1A15 100%)", width: sidebarWidth }}
      >
        {/* Logo */}
        <div className="flex h-16 items-center gap-3 px-6 border-b border-white/[0.06]">
          ...
          {!collapsed && (
            <>
              ...
              <span
                className="ml-auto inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0"
                style={{
        ...
        <div className="px-3 pt-4 pb-2">
          <Link
            ...
            title={collapsed ? "Back to Admin Panel" : undefined}
            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-colors text-white/40 hover:text-white/70 hover:bg-white/[0.04] ${collapsed ? "justify-center px-0" : ""}`}
          >
            <ArrowLeft className="w-3.5 h-3.5 shrink-0" />
            {!collapsed && "Back to Admin Panel"}
          </Link>
        </div>
                  ...
                  return (
                    <li key={item.href}>
                      <Link
                        ...
                        className={`group relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${collapsed ? "justify-center px-0" : ""}`}
                        style={{
        ...
        <div className="px-3 pb-2">
          <button
            ...
            className={`flex items-center gap-3 w-full rounded-lg px-3 py-2.5 text-sm font-medium transition-colors text-white/40 hover:text-white/70 hover:bg-white/[0.04] ${collapsed ? "justify-center px-0" : ""}`}
            title={collapsed ? "Expand sidebar" : "Collapse sidebar"}
          >
            {collapsed ? <PanelLeftOpen className="w-[18px] h-[18px] shrink-0" /> : (
              <>
      ...
      {/* Main */}
      <div className="flex-1 transition-[margin] duration-200" style={{ background: "var(--surface-secondary)", marginLeft: sidebarWidth }}>
        {/* Top bar */}
        <header
